...
elseif ($module=='news' AND $act=='update'){
  if (hakakses($userid,$moduleAkses,'edit')) {  
	$gambar = $_POST['gambar'];
		...
		if (empty($gambar)){
			$update = $db->prepare("UPDATE news SET
										judul = '$_POST[judul]',
										deskripsi = '$_POST[deskripsi]',
										mulai = '$_POST[mulai]',
										tag = '$_POST[tag]', 
			...
			echo " <script src='../../dist/js/jquery-1.11.0.min.js'></script> 
						...
						<script type='text/javascript'>
							$.ajax({
	          					  url: '../../config/fungsi_log2.php',
						          type: 'POST',
						          data: 'userid=$userid&module=$module&aksi=$act&status=editnews',
						     });
    ...
    echo "<script>alert(' Data Gagal Disimpan, Anda Tidak Memiliki Akses !');
           window.location.href='../../media.php?module=$module&act=news'</script>";
  }
}
elseif ($module=='news' AND $act=='hapusnews'){
  
	  	try{
		$db->beginTransaction();	 	
						
			$query = $db->prepare("DELETE FROM news WHERE id_auto='$_POST[id_auto]'");
