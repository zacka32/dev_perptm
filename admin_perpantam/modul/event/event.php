...
$moduleAkses='event';
$module="event";
$aksi="modul/event/aksi_event.php";
switch($_GET['act']){
  // Tampil Modul
  default:
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            if (hakakses($userid,$moduleAkses,'buat')) { ?>
                        <a href="?module=event&act=tambah" class="btn btn-block btn-success pull-right"
                ...
                <div class="box-body">
                    ...
                    <table id="tbl_event" class="display table table-bordered table-striped " width="100%">
                        <thead>
...
function hapusevent(no_event) {
    if (confirm("Apakah Anda yakin ingin menghapus ini")) {
        var userid = <?php echo "$_SESSION[userid]"; ?>;
        var dataString = 'userid=' + userid + '&event_id=' + no_event;
        $.ajax({
            type: 'POST',
            data: dataString,
            url: 'modul/event/aksi_event.php?module=event&act=hapusevent',
...
<?php }else{
    ...
    break;  
  case "editevent":
      if (hakakses($userid,'event','edit')) {
          $id = encrypt_decrypt('decrypt',$_GET['id']);
          $ed=$db->prepare("SELECT * FROM event WHERE id_auto='$id'");
          $ed->execute();        
          $e=$ed->fetch();
    ?>
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                <div class="box-header">
                    <div class="col-xl-6 pull-left">
                        <h3 class="box-title" style="color: #2b31e2;">Edit Kegiatan</h3>
                    </div>
                </div>
                <form autocomplete="off" class="simpanhtml"
                    onSubmit="if(!confirm('Apa yakin ingin simpan data ini')){return false;}"
                    action="<?php echo"$aksi?module=event&act=update";?>" enctype="multipart/form-data" method="POST"
                    class="">
                    <!-- Custom Tabs -->
                    <div class="box-body">

                        <div class="form-group">
                            <label for="inputPassword3" class="col-sm-3 control-label">Judul Kegiatan</label>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group col-md-3" style="background:#fff;">
                                ...
                                <input type="datetime-local" class="form-control"
                                    value="<?= date('Y-m-d\TH:i', strtotime($e['akhir'])) ?>" name="akhir" id="akhir">
                            </div>      
                        </div>
                        <!-- validasi datetime -->          
                        <script>
                        $('#akhir').on('change', function() {   
                            if ($('#mulai').val() && $(this).val() < $('#mulai').val()) {
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                <div class="box-header">
                    <div class="col-xl-6 pull-left">
                        <h3 class="box-title" style="color: #2b31e2;">Entry event</h3>
                    </div>
                </div>
                <form autocomplete="off" class="simpanhtml"
                    onSubmit="if(!confirm('Apa yakin ingin simpan data ini')){return false;}"
                    action="<?php echo"$aksi?module=event&act=input";?>" enctype="multipart/form-data" method="POST"
                    class="">
                    <!-- Custom Tabs -->
                    <div class="box-body">
...
$(document).ready(function() {
    ...
    $('#tbl_event').DataTable({
        ...
        "deferRender": true,
        "ajax": "modul/event/load_data.php?act=list_index",
        "lengthMenu": [
            [10, 25, 50, 100, -1],
            [10, 25, 50, 100, "All"]
        ],
