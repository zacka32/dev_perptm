...
if ($_SESSION['administrator']=='Y'){   
                ...
                while($r = $main->fetch()) {
                    ...
                    if(empty($r['link'])){
                        echo '<a>'.$r['nama_menu'].'</a>';
                            ...
                            echo "<a href='$r[link]'>
                            $r[gambar] <span>$r[nama_menu]</span></a>";
                    }
    ...
    elseif ($_SESSION['administrator']=='N'){
        ...
        $main = $db->prepare("SELECT * FROM mainmenu m left join akses a on m.modul=a.modul
        WHERE m.aktif = 'Y' AND (userid ='$userid' or ID_GROUP='$idgroup') AND lihat='Y' ORDER BY urutan ");
        $main->execute();
          ...
          $sub = $db->prepare("SELECT * FROM submenu s left join akses a on s.modul=a.modul
             WHERE s.id_main = $r[id_main] AND s.aktif='Y' AND (a.userid ='$userid' or a.ID_GROUP='$idgroup') AND lihat='Y' ORDER BY s.urutan ASC
            ");
          $sub->execute();
                    $jml = $sub->rowCount();
                    // apabila sub menu ditemukan
